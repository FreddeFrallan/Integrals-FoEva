...
namespace B83.ExpressionParser
{
	...
	public class ExpressionParser
	{
		...
		public ErrorMessageHandler theErrorLog;
		...
		void SubstitudeBracket(ref string aExpression, int aIndex)
		{
			...
			if (closing > aIndex + 1) {
				...
				GameObject tempHandler = GameObject.FindGameObjectWithTag ("ErrorLog");
				if (tempHandler != null) {
					tempHandler.GetComponent<ErrorMessageHandler> ().setText ("Bracket not closed!");
				}
		...
		IValue Parse(string aExpression)
		{
			...
			if (index2a >= 0 && index2b >= 2)
			{
				...
				if (int.TryParse (inner, out bracketIndex) && bracketIndex >= 0 && bracketIndex < m_BracketHeap.Count) {
					...
					GameObject tempHandler = GameObject.FindGameObjectWithTag ("ErrorLog");
					if (tempHandler != null) {
						tempHandler.GetComponent<ErrorMessageHandler> ().setText ("Can't parse substitude token");
					}
			...
			GameObject tempHandler2 = GameObject.FindGameObjectWithTag ("ErrorLog");
			if (tempHandler2 != null) {
				tempHandler2.GetComponent<ErrorMessageHandler> ().setText ("Reached unexpected end within the parsing tree");
			}
